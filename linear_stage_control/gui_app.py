...
from .stage import ZaberXYStage, list_serial_ports, stage_settings_from_config
...
class CameraDiscoveryWorker(QThread):
    ...
    def run(self) -> None:
        try:
            self.cameras_found.emit(enumerate_cameras(), self.reason)
        except Exception as exc:
...
class MainWindow(QMainWindow):
    ...
    def _build_settings_group(self) -> QGroupBox:
        ...
        self.pixel_format_combo.addItems(["Mono8", "Mono10", "Mono12", "Mono16"])
...
def main() -> int:
    smoke_test = any(arg.lower() == "--smoke-test" for arg in sys.argv[1:]) or os.environ.get(
